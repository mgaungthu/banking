  ...
  useEffect(() => {
    if (isVisible && tokens.length > 0 && testRef.current) {
      ...
      tokens.map((res, index) => {
        if (res.iso_3166_1_alpha2 === selectedCountry) {
          console.log(index);
          scrollIndex = index;
        }
      });
      ...
      <View>
        <View
          ...
          <ThemeText
            ...
            {strings('Country Where you live')}
          </ThemeText>
          <TouchableOpacity
        ...
        <FlatList
          ...
          renderItem={({item, index}) => (
            <TouchableOpacity
              ...
              {item.iso_3166_1_alpha2 === selectedCountry ? (
                <ImageContainer
...
export default SelectLivingCountry;
