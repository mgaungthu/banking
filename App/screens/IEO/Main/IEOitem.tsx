...
const IEOitem = (props: any) => {
  const {currencyName, status, onPress, projectName, startDate, endDate} =
    props;
  ...
  if (statusNew === 'Upcoming' && startDate) {
    timer = (
      <Timer
        style={{fontSize: 12}}
        ms={new Date(startDate).getTime() - Date.now()}
      />
    );
    timerText = 'Starts In:';
  ...
  return (
    <Cell
      style={[styles.container, isRtlApproach ? rtlStyles.reverseRow : {}]}
      onPress={onPress}>
      ...
      <View
        style={[
          styles.currencyInfoContainer,
          isRtlApproach ? rtlStyles.reverseRow : {},
        ]}>
        <View>
          <Text style={[styles.title, textColor()]}>{currencyName}</Text>
          <Text
            style={[
              styles.subTitle,
