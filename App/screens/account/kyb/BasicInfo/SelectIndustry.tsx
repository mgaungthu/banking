...
const SelectIndustry = ({
  ...
  const {industries} = useSelector((state: any) => state.appReducer);
  ...
  const updateIndustry = () => {
    if (industries.length > 0) {
      const filteredData = industries?.sort((a, b) =>
  ...
  const searchText = e => {
    ...
    setSearchQuery(e);
    if (industries?.length > 0) {
      let countryList = industries.sort((a, b) => a.name.localeCompare(b.name));
      ...
      <View>
        <View
          ...
          <ThemeText
            ...
            {`${strings('select')} ${strings('industries')}`}
          </ThemeText>
          <TouchableOpacity
        ...
        <FlatList
          ...
          renderItem={({item, index}) => (
            <TouchableOpacity
              key={index}
              activeOpacity={1}
              onPress={handleIndustry(item, index)}
              style={
                isRtlApproach
