...
const SelectWallet = ({
  ...
  const {pairs} = useSelector((state: any) => state.quickBuyReducer);
  ...
  const updateList = () => {
    if (quickBuyData?.fundsList?.length > 0) {
      const filteredData = filteredWallets();
  ...
  const filteredWallets = () => {
    const symbol = CardTypeCurrency;

    return quickBuyData?.fundsList
      ?.filter(wallet => {
        return wallet.currency.card_order_enabled;
      })
  ...
  const searchText = e => {
    ...
    setSearchQuery(e);
    if (quickBuyData?.fundsList?.length > 0) {
      let currencyList = filteredWallets();
              ...
              <TouchableOpacity
                ...
                style={
                  isRtlApproach
                    ...
                    : [styles.rowItem, item.disabled && {opacity: 0.3}]
                }>
